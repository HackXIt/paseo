...
Import discovery includes live Pi workers related to a Herdr target in the requested workspace even when a worker uses another working directory. Importing that worker places it in a workspace for the worker's directory. Paseo assigns the imported worker's parent only when Herdr supplies directional parent-target metadata; sharing a Herdr workspace does not establish parentage.
